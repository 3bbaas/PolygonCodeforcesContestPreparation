...
public:
    ...
    string getBinary(int size, bool DT = false)
    {
        string ret;
        while (size--)
            ret += to_string(get(0, 1));
        if (DT)
            return stoll(ret), ret;
        return ret;
    }
};
/* +:-:+:-:+:-:+:-:+:-:+:-:+:-[ Gen-Class ]:-:+:-:+:-:+:-:+:-:+:-:+:-+:- */
...
signed main(int argc, char **argv)
{
    ...
    {
        string FILE = R"(tests\testCase-)" + to_string(i) + ".txt";
